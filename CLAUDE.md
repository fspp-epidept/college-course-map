...
- **IPC types:** [tauri-specta](https://github.com/specta-rs/tauri-specta) + `specta-typescript` — Rust `#[tauri::command]` handlers generate a typed `bindings.ts` consumed by the frontend instead of stringly-typed `invoke()` calls.
