...
def verify_one(
    ...
    for text in inputs:
        ...
        # Record per-input ONNX outputs so the Rust side can assert identical
        # behaviour from its own ORT session against this fixture. App-active
        # specs only, keyed by the app-facing install dir (check_parity.rs
        # loads models from those dirs and bails on names it doesn't know).
        if spec.app_subdir is not None:
            per_input.append({
                "model_subdir": spec.app_subdir,
                "input": text,
